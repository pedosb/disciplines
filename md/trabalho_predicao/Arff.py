...
if __name__=="__main__":
   ...
   new_arff.matrix['data'] = []
   new_arff.matrix['fluxo'] = []
   for index in range(len(arff.matrix['data'])):
      mes = arff.matrix['data'][index][3:10]
      try:
	 index_in = new_arff.matrix['data'].index(mes)
      except ValueError:
	 new_arff.matrix['data'].append(mes)
	 new_arff.matrix['fluxo'].append(0)
	 index_in = len(new_arff.matrix['data'])-1
      new_arff.matrix['fluxo'][index_in] += int(arff.matrix['fluxo'][index])
   new_arff.write(OUT_ARFF_FILE)
